...
void MainWindow::initializeGL()
{
    ...
    _camera.lookAt(glm::vec3(2.0f, 3.0f, 10.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    _squareCase.reset(new SquareCase(_glcore));
    _squareCase->init();

    _squareLightSource.reset(new SquareLightSource(_glcore));
    _squareLightSource->init();
...
void MainWindow::paintGL()
{
    ...
    _squareCase->setModel(glm::mat4(1.0f));
    _squareCase->setView(_camera.getView());
    _squareCase->setProjection(_projection);
    _squareCase->paint();

    glm::mat4 model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(2.f, 2.f, 2.f));

    _squareLightSource->setModel(model);
    _squareLightSource->setView(_camera.getView());
    _squareLightSource->setProjection(_projection);
    _squareLightSource->paint();
}
